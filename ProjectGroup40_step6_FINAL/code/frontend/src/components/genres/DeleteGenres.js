...
function DeleteGenre() {
  ...
  const handleDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`http://flip3.engr.oregonstate.edu:35281/api/genres/${ location.state.genre_id }`);
      setLoading(false);
      navigate('/genres');
    } catch (error) {
      console.error(error);
      alert('Cannot delete since it is crossreferences in Movies and Genres, delete there first.');
      setLoading(false);
    }
  };
